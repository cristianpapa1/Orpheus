...
/**
 * The store-setup notice for the catalog crawler. It always tells the seller
 * where they stand — validated, pending, or not yet claimed — so when they reach
 * the store-creation phase they know *why* the crawler is (un)available and
 * *what* is pending. Only a validated institution with an open store sees the
 * working crawler; everyone else sees the path to unlock it, and can still add
 * products by hand.
 */
export function CrawlerNotice({
  validation,
  hasStore,
}: {
  ...
  if (status === "validated") {
    ...
    if (hasStore) {
      // Store is open + validated → the crawler is live.
      return (
        <div className="mt-10">
    ...
    return (
      <div className="mt-6 border-2 border-ink bg-yellow px-4 py-3">
        ...
        <p className="mt-1 text-body">
          ...
          your existing catalog automatically with the crawler.
        </p>
      </div>
    );
  }
      ...
      <div className="flex flex-col gap-3 p-4">
        ...
        {isPending ? (
          <p className="text-body">
            The crawler pulls your existing catalog in for you — but it only runs
            for the validated institution behind a shop. Your claim to{" "}
            <strong>{pendingName}</strong> is under review. As soon as an admin
            validates you on Atelier, the crawler unlocks right here.
          </p>
        ) : (
          <p className="text-body">
            The crawler pulls your existing catalog in for you — but it only runs
            for validated institutions, so only the real owner of a shop can
            scrape it. Find your institution on Atelier and claim it; once an
            admin validates you, the crawler appears here automatically.
          </p>
        )}
